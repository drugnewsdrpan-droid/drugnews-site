(async function () {
  ...
  function resultHref(item) { return /^https?:\/\//i.test(item.url) ? item.url : `/${String(item.url || '').replace(/^\/+/, '')}`; }
  function categoryDisplay(category = '') { return category === '商業分析系列' ? 'Business Analysis' : category || 'Analysis'; }
  function card({ item, reasons }, rank) { const image = item.image || ''; return `<a class="article-card${image ? ' with-image' : ''}" href="${esc(resultHref(item))}">${image ? `<div class="thumb-wrap"><img class="card-thumb" src="${esc(image)}" alt="${esc(item.imageAlt || item.title)}" loading="lazy"></div>` : ''}<div class="article-card-body"><div class="meta"><span>#${rank + 1}</span><span>${esc(item.date)}</span><span>${esc(categoryDisplay(item.category))}</span></div><h3>${esc(item.title)}</h3><p>${esc(item.summary || '')}</p>${reasons.length ? `<div class="reason-row">${reasons.slice(0, 3).map((reason) => `<span>${esc(reason)}</span>`).join('')}</div>` : ''}</div></a>`; }
